...
streamClientWidget::streamClientWidget(QString host, int port, QWidget *parent)
    ...
    setWindowTitle("This is QTCPServer Window!");
    m_pSocket = new QTcpSocket(this);
    // connect (m_pSocket,SIGNAL (connected()), this, SLOT (on_connected()));
    connect(m_pSocket, &QIODevice::readyRead, this, &streamClientWidget::on_readyRead, Qt::DirectConnection);
    connect(m_pSocket, &QAbstractSocket::connected, this, &streamClientWidget::on_connected);
    connect(m_pSocket, &QAbstractSocket::disconnected, this, &streamClientWidget::on_disconnected);

    m_pTimer = new QTimer(this);
    connect(m_pTimer, &QTimer::timeout, this, &streamClientWidget::on_timeout);
    m_pTimer->start(1000);
}
