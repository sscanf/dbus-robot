...
facetrackerWorker::facetrackerWorker(const QString &strName, const QString &strDescription, bool bEnabled, QObject *parent)
    : QObject(parent)
    , m_strName(strName)
    ...
    new facetracker_workerInterface(this);
    ...
    connect(this, &facetrackerWorker::nextTrack, this, &facetrackerWorker::on_track);
    ...
    connect(m_pSocket, &QTcpServer::newConnection, this, &facetrackerWorker::on_newConnection);
...
void facetrackerWorker::morphOps(Mat const &thresh) {
...
void facetrackerWorker::startTracking() {
    emit nextTrack();
    //    m_pTimer->start (10);
}

void facetrackerWorker::stopTracking() {
    //    m_pTimer->stop();
}

void facetrackerWorker::startStream() {
    m_pThresholdSender->startListening();
    m_pResultSender->startListening();
}

void facetrackerWorker::stopStream() {
    m_pThresholdSender->stopListening();
    m_pResultSender->stopListening();
}

QPoint facetrackerWorker::centerDistance() {
    return m_centerDistance;
}

bool facetrackerWorker::isBallDetected() {
    return m_bBallDetected;
}

QPoint facetrackerWorker::xyPossition() {
    QPoint pos;
...
int facetrackerWorker::zPossition() {
    return m_centerBall.z();
}

void facetrackerWorker::drawCVPannel() {
...
// void facetrackerWorker::on_timeout()
//{
//    detectCircles();
//}

void facetrackerWorker::detectCircles() {
    Mat imgThresholded;
    m_bBallDetected = false;
...
void facetrackerWorker::on_newConnection() {
...
void facetrackerWorker::on_disconnected() {
    m_pClient->deleteLater();
}

void facetrackerWorker::on_readyRead() {
...
void facetrackerWorker::on_track() {
    detectCircles();
}
