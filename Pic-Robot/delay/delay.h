...
//delay routine
	...
	#define Jumpback _asm("goto $ - 4") _endasm; //on PIC18F, it is asm("goto $ - 4") and on PIC16F core it is asm("goto $ - 2")
#elif PIC_CLK == 8000000
	...
	#define WaitFor1Us{\
			 ...
			 nop \
			_endasm;}
	#define Jumpback _asm goto $-4 _endasm;
#elif PIC_CLK == 16000000
	#define DelayDivisor 1
	#define WaitFor1Us _asm("nop") _endasm;
 	#define Jumpback _asm("goto $ - 4")
#elif PIC_CLK == 20000000
	#define DelayDivisor 1
	#define WaitFor1Us _asm("nop"); _asm("nop")
	#define Jumpback _asm("goto $ - 6")
#elif PIC_CLK == 32000000
	#define DelayDivisor 1
	#define WaitFor1Us _asm nop nop nop nop nop _endasm;
	#define Jumpback _asm goto $ - 12 _endasm;
#else
	#error delay.h - please define PIC_CLK correctly
#endif
...
#define DelayUs(x) { \
			...
			_asm("movlb (_delayus_variable) >> 8"); _endasm;\
			WaitFor1Us; } \
			_asm("decfsz (_delayus_variable)&0ffh,f"); _endasm;\
			Jumpback;
