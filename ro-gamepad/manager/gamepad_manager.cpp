...
gamepadManager::gamepadManager(int & argc, char ** argv) :
    QCoreApplication(argc, argv),
    m_connection (QDBusConnection::systemBus())
{
    new gamepad_managerInterface(this);
    m_connection.registerService("com.robot.rogamepad");
    m_connection.registerObject("/rogamepad",this);
    parseConfig (GAMEPAD_CONFIG_FILE);
}

QStringList gamepadManager::getObjects()
{
  QStringList strObjects;
  foreach (QPluginLoader *pLoader, m_hPlugins) {
    gamepadPlugin *pPlugin = qobject_cast<gamepadPlugin*>(pLoader->instance());
    strObjects << pPlugin->getAllObjects ();
  }
  return strObjects;
}

QStringList gamepadManager::getObjectsByType(QString strType)
{
  ...
  foreach (QString object, strAllObjects) {
    ...
    QDBusInterface interface ("com.robot.gamepad",
                              object,
                              strInterface,
                              bus,
                              this);
...
QStringList gamepadManager::getObjectsByName(QString strName)
{
  QStringList strAllObjects = getObjects();
  QStringList strObjects;
...
QPluginLoader *gamepadManager::loadPlugin (const char *pstrPlugin)
{
...
int gamepadManager::parseConfig(QString strFilePath)
{
  QDomDocument doc ("gamepad");
  QString      errorMsg;
  int          errorLine;
  int          errorColumn;
  ...
  while (!config.isNull()) {
    ...
    pZPlugin = qobject_cast <gamepadPlugin*>( pPlugin->instance ());
    if (!pZPlugin)
      return EXIT_FAILURE;
